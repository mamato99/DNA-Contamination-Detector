...
class AVLTree:
    ...
    def insert(self,value, data):
        if self.root==None:
            ...
            self.node_max = self.root
            self.len += 1
        else:
            self._insert(value, data, self.root)

    def _insert(self, value, data, cur_node):
        if value<cur_node.value:
            if cur_node.left_child==None:
                cur_node.left_child=node(value, data)
                cur_node.left_child.parent=cur_node # set parent
                cur_node.left_child.before = self.before(cur_node.left_child)
                self._inspect_insertion(cur_node.left_child)
            else:
                self._insert(value, data, cur_node.left_child)
        elif value>=cur_node.value:
            if cur_node.right_child==None:
                cur_node.right_child=node(value, data)
                cur_node.right_child.parent=cur_node # set parent
                cur_node.right_child.before = self.before(cur_node.right_child)
                self._inspect_insertion(cur_node.right_child)

                if value >= self.node_max.value:
                    self.node_max = cur_node.right_child
            else:
                self._insert(value, data, cur_node.right_child)
        self.len += 1
    ...
    def _subtree_first_position(self, p):
        ...
        walk = p
        while walk.left_child is not None:                 # keep walking left
    ...
    def _subtree_last_position(self, p):
        ...
        walk = p
        while walk.right_child is not None:                # keep walking right
            walk = walk.right_child
        return walk

    def before(self, p):
        if p.left_child:
            return self._subtree_last_position(p.left_child)
        else:
            # walk upward
            walk = p
            above = walk.parent
            while above is not None and walk == above.left_child:
                walk = above
                above = walk.parent
            return above
    ...
    def after(self, p):
        ...
        if p.right_child:
            return self._subtree_first_position(p.right_child)
        else:
            walk = p
            above = walk.parent
            while above is not None and walk == above.right_child:
                walk = above
                above = walk.parent
            return above